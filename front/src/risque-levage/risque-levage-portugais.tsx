...
const RisqueLevagePortugais: React.FC = () => {
  const navigate = useNavigate();
  const audioRef = useRef<HTMLAudioElement>(null);

  const titleTexts: Record<string, string> = {
    pt: 'Risco de elevação',
  };

  // Harmonisation du chemin : racine "/" + structure de nommage
      ...
      <main className="risque-levage-content">
        <div className="content-wrapper">
          <div className="image-section">
            <img src="/ressources/grue.png" alt="Elevação" className="grue-image" />
          </div>
          <div className="text-section">
            <h1>{titleTexts.pt}</h1>
            <div className="description">
              <p>
                Perigos associados a operações de elevação (guindastes, gruas, carrinhos, etc.), que podem levar à queda de objetos, esmagamento ou colisões.
              </p>
            </div>
          </div>
        </div>
...
export default RisqueLevagePortugais;